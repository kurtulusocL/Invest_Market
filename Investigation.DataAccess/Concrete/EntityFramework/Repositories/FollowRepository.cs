...
{
    public class FollowRepository : EntityRepositoryBase<Follow, ApplicationDbContext>, IFollowRepository
    {
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<Follow>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
