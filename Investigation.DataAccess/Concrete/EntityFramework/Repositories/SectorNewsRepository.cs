...
{
    public class SectorNewsRepository : EntityRepositoryBase<SectorNews, ApplicationDbContext>, ISectorNewsRepository
    {
        readonly ApplicationDbContext _context;
        public SectorNewsRepository(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        public async Task<SectorNews?> GetBySlugAsync(string slug)
        {
            try
            {
                ...
                var candidates = await _context.SectorNews
                    .AsNoTracking()
                ...
                return await _context.SectorNews/*.Include(n => n.Reports).Include(n => n.SavedContents)*/.FirstOrDefaultAsync(n => n.Id == match.Id && n.IsActive && !n.IsDeleted);
            }
            catch (Exception ex)
            {
        ...
        public SectorNews HitRead(int id)
        {
            try
            {
                var hitRead = _context.Set<SectorNews>().Where(i => i.Id == id).FirstOrDefault();
        ...
        public async Task<bool> LikeAsync(int id)
        {
            try
            {
                var like = await _context.Set<SectorNews>().Where(i => i.Id == id).FirstOrDefaultAsync();
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<SectorNews>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<SectorNews>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
