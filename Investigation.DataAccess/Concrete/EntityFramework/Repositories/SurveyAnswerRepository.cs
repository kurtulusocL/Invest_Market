...
{
    public class SurveyAnswerRepository : EntityRepositoryBase<SurveyAnswer, ApplicationDbContext>, ISurveyAnswerRepository
    {
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<SurveyAnswer>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<SurveyAnswer>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
        ...
        public int SurveyAnswerCounter()
        {
            try
            {
                return _context.SurveyAnswers.Count();
            }
            catch (Exception)
            {
                return 0;
