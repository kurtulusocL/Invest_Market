...
{
    public class EventsCategoryRepository : EntityRepositoryBase<EventsCategory, ApplicationDbContext>, IEventsCategoryRepository
    {
        readonly ApplicationDbContext _context;
        public EventsCategoryRepository(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<EventsCategory>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<EventsCategory>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
