...
{
    public class CompanyStageRepository : EntityRepositoryBase<CompanyStage, ApplicationDbContext>, ICompanyStageRepository
    {
        readonly ApplicationDbContext _context;
        public CompanyStageRepository(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var data = await _context.Set<CompanyStage>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (data == null)
                {
                    return false;
                }
                ...
                var hits = await _context.Set<Hit>().Where(a => a.CompanyStageId == id).ToListAsync();
