...
{
    public class CompanyTeamRepository : EntityRepositoryBase<CompanyTeam, ApplicationDbContext>, ICompanyTeamRepository
    {
        readonly ApplicationDbContext _context;
        public CompanyTeamRepository(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<CompanyTeam>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<CompanyTeam>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
