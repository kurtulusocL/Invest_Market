...
{
    public class SavedContentRepository : EntityRepositoryBase<SavedContent, ApplicationDbContext>, ISavedContentRepository
    {
        ...
        public int SavedContentCounter()
        {
            try
            {
                return _context.SavedContents.Count();
            }
            catch (Exception)
            {
                return 0;
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<SavedContent>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<SavedContent>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
