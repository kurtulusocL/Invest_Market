...
{
    public class LogoRepository : EntityRepositoryBase<Logo, ApplicationDbContext>, ILogoRepository
    {
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<Logo>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<Logo>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
