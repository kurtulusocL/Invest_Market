...
{
    public class CompanyFinanceRepository : EntityRepositoryBase<CompanyFinance, ApplicationDbContext>, ICompanyFinanceRepository
    {
        readonly ApplicationDbContext _context;
        public CompanyFinanceRepository(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var data = await _context.Set<CompanyFinance>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (data == null)
                {
                    return false;
                }
                ...
                var hits = await _context.Set<Hit>().Where(a => a.CompanyFinanceId == id).ToListAsync();
