...
{
    public class CommentRepository : EntityRepositoryBase<Comment, ApplicationDbContext>, ICommentRepository
    {
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var data = await _context.Set<Comment>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (data == null)
                {
                    return false;
                }
                ...
                var commentAnswers = await _context.Set<Hit>().Where(a => a.CommentId == id).ToListAsync();
                foreach (var commentAnswer in commentAnswers)
                {
                    commentAnswer.IsActive = true;
                }

                var hits = await _context.Set<Hit>().Where(a => a.CommentId == id).ToListAsync();
                foreach (var hit in hits)
                {
                    hit.IsActive = true;
                }

                var likes = await _context.Set<Hit>().Where(a => a.CommentId == id).ToListAsync();
                foreach (var like in likes)
                {
                    like.IsActive = true;
                }

                var reports = await _context.Set<Report>().Where(a => a.CommentId == id).ToListAsync();
