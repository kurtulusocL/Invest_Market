...
{
    public class SendMessageRepository : EntityRepositoryBase<SendMessage, ApplicationDbContext>, ISendMessageRepository
    {
        readonly ApplicationDbContext _context;
        public SendMessageRepository(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<SendMessage>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<SendMessage>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
