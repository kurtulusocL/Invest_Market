...
{
    public class CompanyPintechRepository : EntityRepositoryBase<CompanyPintech, ApplicationDbContext>, ICompanyPintechRepository
    {
        readonly ApplicationDbContext _context;
        public CompanyPintechRepository(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var data = await _context.Set<CompanyPintech>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (data == null)
                {
                    return false;
                }
                ...
                var hits = await _context.Set<Hit>().Where(a => a.CompanyPintechId == id).ToListAsync();
